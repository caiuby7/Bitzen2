...
function currentTime() {
    var e = $(".stats .icon-calendar").parent(),
        t = new Date,
        n = ["Janeiro", "Fevereiro", "Mar&ccedil;o", "Abril", "Maio", "Junho", "Julho", "Agosto", "Setembro", "Outubro", "Novembro", "Dezembro"],
        r = ["Domingo", "Segunda", "Ter&ccedil;a", "Quarta", "Quinta", "Sexta", "S&aacute;bado"];
    e.find(".details .big").html( t.getDate() + " de " + n[t.getMonth()] + " de " + t.getFullYear());
    e.find(".details span").last().html(r[t.getDay()] + ", " + t.getHours() + ":" + ("0" + t.getMinutes()).slice(-2) + ":" + ("0" + t.getSeconds()).slice(-2));
    setTimeout(function () {
        currentTime()
    }, 1e3)
}
...
$(document).ready(function () {
    ...
    randomFeed();
    var e = [];
    for (var t = 0; t <= 20; t += 1) e.push([t, parseInt(32 + Math.random() * 35)]);
    var n = [];
    for (var t = 0; t <= 20; t += 1) n.push([t, parseInt(32 + Math.random() * 55)]);
    var r = [];
    for (var t = 0; t <= 10; t += 1) r.push([t, parseInt(31 + Math.random() * 7)]);
    var i = new Array;
    i.push({
    ...
    if ($("#flot-3").length > 0) {
        ...
        var g = 30;
        $("#updateInterval").val(g).change(function () {
            var e = $(this).val();
            if (e && !isNaN(+e)) {
                g = +e;
                g < 1 && (g = 1);
    ...
    if ($("#flot-1").length > 0) {
        var w = [];
        for (var t = 0; t < Math.PI * 2; t += .25) w.push([t, Math.sin(t)]);
        var E = [];
        for (var t = 0; t < Math.PI * 2; t += .25) E.push([t, Math.cos(t)]);
        var S = [];
        for (var t = 0; t < Math.PI * 2; t += .1) S.push([t, Math.tan(t)]);
        $.plot($("#flot-1"), [{
            label: "sin(x)",
            data: w
        }, {
    ...
    (location.pathname == "/flat/" || location.pathname == "/flat" || location.pathname == "/flat/index.html") && bootbox.confirm("Would you like to start the page guide? It will show you functions you could miss!", "No", "Yes", function (e) {
        e && $.pageguide(k, {
            events: {
                close: function () {
                    $.pageguide("unload")
